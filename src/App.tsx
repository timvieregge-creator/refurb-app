...
function App() {
  ...
  const suppliers = useMemo(
    () => [...new Set(batches.map((b) => b.supplier_name))].filter(Boolean).sort((a, b) => a.localeCompare(b, "de")),
    [batches],
  );
...
if (
  ...
  const deviceCounts = useMemo(() => {
    const result = {} as Record<DeviceStatus, number>;
    for (const [status] of statusOptions) result[status] = 0;
    for (const device of allDevices) if (device.status in result) result[device.status] += 1;
    return result;
  }, [allDevices]);
  ...
  async function loadInitialData() {
    try {
      setLoading(true);
      setError("");
      const [loadedBatches, loadedDevices] = await Promise.all([getSourceBatches(), getAllDevices()]);
  ...
  async function refreshDevices(batchId = selectedBatchId) {
    const loaded = await getAllDevices();
    setAllDevices(loaded);
    setSearchResults(deviceSearch.trim() ? await searchDevices(deviceSearch) : loaded);
    if (batchId) setDevices(await getDevicesByBatch(batchId));
  }

  function resetBatchForm() {
    setEditingBatchId(null); setSupplierName(""); setSupplierReference(""); setLotNumber(""); setDeliveryNoteNumber(""); setReceivedAt(today()); setTotalPurchaseCost(""); setTransportCost(""); setNotes("");
  }

  function startBatchEditing(b: SourceBatch) {
  ...
  async function handleBatchSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!supplierName.trim()) return setError("Bitte einen Lieferanten eingeben.");
    const input = { supplierName, supplierReference, lotNumber, deliveryNoteNumber, receivedAt, totalPurchaseCost: parseNumber(totalPurchaseCost), transportCost: parseNumber(transportCost), notes };
    try { setSaving(true); setError(""); if (editingBatchId) await updateSourceBatch(editingBatchId, input); else { const batch = await createSourceBatch(input); setSelectedBatchId(batch.id); setDevices([]); } resetBatchForm(); setBatches(await getSourceBatches()); } catch (err) { setError(err instanceof Error ? err.message : "Fehler beim Speichern"); } finally { setSaving(false); }
  }
  ...
  function resetDeviceForm() {
    setEditingDeviceId(null); setSerialNumber(""); setImei1(""); setImei2(""); setManufacturer(""); setModel(""); setDeviceType("notebook"); setPurchaseCost(""); setCondition("unknown"); setDefectCategory(""); setDefectDescription(""); setAccessoriesComplete(""); setDataErasureStatus("unknown"); setInspectionNotes("");
  }

  function startDeviceEditing(d: Device) {
  ...
  function deviceInput() {
    return { serialNumber, imei1, imei2, manufacturer, model, deviceType, purchaseCost: parseNumber(purchaseCost), condition, defectCategory, defectDescription, accessoriesComplete: accessoriesComplete === "" ? null : accessoriesComplete === "yes", dataErasureStatus, inspectionNotes };
  }
  ...
  useEffect(() => {
    function afterPrint() { document.body.classList.remove("printing"); setPrintDevice(null); }
  ...
  return (
    <>
      ...
      {printDevice && <div className="print-card"><div className="print-card-content">
<QRCodeSVG
  value={`${window.location.origin}/#/geraet/${encodeURIComponent(printDevice.internal_number)}`}
  size={180}
  level="M"
  includeMargin
...
const styles = {
  main: { maxWidth: 1200, margin: "0 auto", padding: 32, fontFamily: "Arial, sans-serif" },
  section: { border: "1px solid #ddd", borderRadius: 8, padding: 20, marginTop: 24, marginBottom: 32 },
  grid: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16, marginTop: 16 },
  filterGrid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 20 },
  dashboardGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginTop: 16 },
  dashboardCard: { display: "flex", flexDirection: "column" as const, gap: 6, padding: 16, border: "1px solid #ddd", borderRadius: 8, background: "#f8fafc" },
  input: { display: "block", width: "100%", boxSizing: "border-box" as const, marginTop: 6, padding: 10, border: "1px solid #bbb", borderRadius: 5 },
  textarea: { display: "block", width: "100%", minHeight: 80, boxSizing: "border-box" as const, marginTop: 6, padding: 10, border: "1px solid #bbb", borderRadius: 5, resize: "vertical" as const },
  buttonRow: { display: "flex", gap: 10, alignItems: "center" },
  blueButton: { marginTop: 20, padding: "10px 18px", border: 0, borderRadius: 6, background: "#2563eb", color: "white", cursor: "pointer" },
  greenButton: { marginTop: 20, padding: "10px 18px", border: 0, borderRadius: 6, background: "#16a34a", color: "white", cursor: "pointer" },
  grayButton: { marginTop: 20, padding: "10px 18px", border: 0, borderRadius: 6, background: "#6b7280", color: "white", cursor: "pointer" },
  smallBlueButton: { padding: "6px 10px", border: 0, borderRadius: 5, background: "#2563eb", color: "white", cursor: "pointer" },
  smallRedButton: { padding: "6px 10px", border: 0, borderRadius: 5, background: "#dc2626", color: "white", cursor: "pointer" },
  smallGrayButton: { padding: "6px 10px", border: 0, borderRadius: 5, background: "#64748b", color: "white", cursor: "pointer" },
  statusSelect: { padding: 6, borderRadius: 5, border: "1px solid #bbb" },
  error: { background: "#fee2e2", color: "#991b1b", padding: 12, borderRadius: 6, marginTop: 20 },
  table: { width: "100%", borderCollapse: "collapse" as const },
  cell: { textAlign: "left" as const, padding: 10, borderBottom: "1px solid #ddd", verticalAlign: "top" as const },
};
